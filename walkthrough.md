...
- **KPIs**: Métricas principais calculadas em tempo real.
- **Gráficos**: Funil de conversão e evolução diária.
- **Tabelas**: Top origens e últimas vendas.
- **Sair**: Botão de logout no topo.

### 3. Developer Tools (`/dashboard/developer`)

- **Conexão API**: Visualize a URL de ingestão e a Chave Secreta (Bearer Token).
- **Logs em Tempo Real**: Monitore eventos chegando ao vivo (Pageviews, Checkouts, Compras).
- **Visualizador JSON**: Clique em "Payload" para inspecionar os dados brutos do evento.

### 4. Login (`/login`)

- Acesse com seu email e senha do Supabase Auth.
- Redireciona automaticamente para o Dashboard após sucesso.

## Notas Técnicas

- **Middleware**: Protege rotas (`/dashboard`) verificando sessão no servidor.
- **SSR Auth**: Usa `@supabase/ssr` para gerenciar cookies de sessão.
- **Realtime**: Usa Supabase Realtime para atualizar os logs instantaneamente.
